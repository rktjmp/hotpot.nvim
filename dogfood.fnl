...
(fn compile-fresh [cache-dir fnl-dir]
  ...
  (fn compile-dir [fennel in-dir out-dir relative-path]
    (let [scanner (uv.fs_scandir in-dir)]
      (var ok true)
      (each [name type #(uv.fs_scandir_next scanner) :until (not ok)]
        (match type
          "directory" (let [out-down (.. cache-dir :/ in-dir :/ name)
                            in-down (.. in-dir :/ name)
                            relative-down (.. relative-path :/ name)]
                        (vim.fn.mkdir out-down :p)
                        (compile-dir fennel in-down out-down relative-down))
          "file" (let [out-file (.. out-dir :/ (string.gsub name ".fnl$" ".lua"))
                       in-file  (.. in-dir :/ name)]
                   (when (not (= name :macros.fnl))
                     (local modname (path-to-modname (.. relative-path :/ name)))
