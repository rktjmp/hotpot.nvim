...
(let [cache-dir (vim.fs.joinpath (vim.fn.stdpath :data) :site :pack :hotpot :opt :config)]
  (case (vim.uv.fs_stat cache-dir)
    ;; Missing, create so nvim can find it on subsequent boots, try first sync if possible.
    nil (let [_ (vim.fn.mkdir cache-dir "p")
    ...
    {:type t} (let [msg  "Hotpot: %s exists but is not directory, is %s, consider removing it?"]
                (vim.notify (string.format msg cache-dir t) vim.log.levels.ERROR {}))))
