package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_598_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _609_, _610_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_609_ == true) and (nil ~= _610_)) then
      local old = _610_
      local _
      ...
      return on_values({"ok"})
    elseif ((_609_ == false) and (nil ~= _610_)) then
      local msg = _610_
      if (specials["macro-loaded"])[module_name] then
        ...
        local function _615_()
          local _614_ = msg:gsub("\n.*", "")
          return _614_
        end
        return on_error("Runtime", _615_())
      end
    else
      return nil
    end
  ...
  local function run_command(read, on_error, f)
    local _618_, _619_, _620_ = pcall(read)
    if ((_618_ == true) and (_619_ == true) and (nil ~= _620_)) then
      local val = _620_
      return f(val)
    elseif (_618_ == false) then
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _624_ = type(subtbl)
        if (_624_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_624_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _627_, names)
          else
          end
        else
        end
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      do
        ...
        if ("function" == type(tgt)) then
          local _636_ = (compiler.metadata):get(tgt, "fnl/docstring")
          if (nil ~= _636_) then
            local docstr = _636_
  ...
  local function resolve(identifier, _643_, scope)
    local _arg_644_ = _643_
    local ___replLocals___ = _arg_644_["___replLocals___"]
    local env = _arg_644_
    local e
    local function _645_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _645_})
    local _646_, _647_ = pcall(compiler["compile-string"], tostring(identifier), {scope = scope})
    if ((_646_ == true) and (nil ~= _647_)) then
      local code = _647_
      local _648_ = specials["load-code"](code, e)()
      local function _649_()
        local x = _648_
        return (type(x) == "function")
      end
      if ((nil ~= _648_) and _649_()) then
        local x = _648_
        return x
      else
        return nil
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _652_(_241)
      ...
      if ((_G.type(_653_) == "table") and ((_653_).what == "Lua") and (nil ~= (_653_).short_src) and (nil ~= (_653_).linedefined) and (nil ~= (_653_).source)) then
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _666_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _666_) then
          local cmd_name = _666_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _668_ = commands[command_name]
      if (nil ~= _668_) then
        local command = _668_
      ...
      do
        ...
        do
          ...
          for k, v in pairs(readline.set_options({})) do
            local _673_, _674_ = k, v
            if ((nil ~= _673_) and (nil ~= _674_)) then
              local k_12_auto = _673_
              local v_13_auto = _674_
  ...
  local function repl(_3foptions)
    ...
    local function _680_(parser_state)
      local c = byte_stream(parser_state)
      table.insert(chars, c)
      return c
    end
    read, reset = parser.parser(_680_)
    ...
    if opts.registerCompleter then
      local function _684_()
        local _682_ = env
        local _683_ = opts.scope
        local function _685_(...)
          return completer(_682_, _683_, ...)
        end
        return _685_
      end
      opts.registerCompleter(_684_())
    else
    end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if not_eof_3f then
          do
            local _689_, _690_ = nil, nil
            local function _692_()
              local _691_ = opts
              _691_["source"] = src_string
              return _691_
            end
            _689_, _690_ = pcall(compiler.compile, x, _692_())
            if ((_689_ == false) and (nil ~= _690_)) then
              local msg = _690_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_689_ == true) and (nil ~= _690_)) then
              local src = _690_
              local src0
              if save_locals_3f then
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function wrap_env(env)
    ...
    local function _392_(_, key, value)
      if utils["string?"](key) then
    ...
    local function _394_()
      local function putenv(k, v)
        ...
        if utils["string?"](k) then
          _395_ = compiler["global-unmangling"](k)
        else
          _395_ = k
        end
        return _395_, v
    ...
    return setmetatable({}, {__index = _390_, __newindex = _392_, __pairs = _394_})
  end
  local function current_global_names(_3fenv)
    local mt
    do
      local _397_ = getmetatable(_3fenv)
      if ((_G.type(_397_) == "table") and (nil ~= (_397_).__pairs)) then
        local mtpairs = (_397_).__pairs
        local tbl_11_auto = {}
        for k, v in mtpairs(_3fenv) do
          local _398_, _399_ = k, v
          if ((nil ~= _398_) and (nil ~= _399_)) then
            local k_12_auto = _398_
            local v_13_auto = _399_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
        mt = tbl_11_auto
      elseif (_397_ == nil) then
        mt = (_3fenv or _G)
      else
        mt = nil
      end
  ...
  local function load_code(code, _3fenv, _3ffilename)
    ...
    local _402_, _403_ = rawget(_G, "setfenv"), rawget(_G, "loadstring")
    if ((nil ~= _402_) and (nil ~= _403_)) then
      local setfenv = _402_
      local loadstring = _403_
  ...
  local function doc_2a(tgt, name)
    if not tgt then
      ...
      if ((type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function"))) then
        ...
        return string.format("(%s%s%s)\n  %s", name, _406_, arglist, docstring)
      else
  ...
  local function set_fn_metadata(arg_list, docstring, parent, fn_name)
    if utils.root.options.useMetadata then
      local args
      local function _420_(_241)
        return ("\"%s\""):format(deep_tostring(_241))
      end
      args = utils.map(arg_list, _420_)
  ...
  local function get_fn_name(ast, scope, fn_name, multi)
    if (fn_name and (fn_name[1] ~= "nil")) then
      local _423_
      if not multi then
        _423_ = compiler["declare-local"](fn_name, {}, scope, ast)
      else
        _423_ = (compiler["symbol-to-expression"](fn_name, scope))[1]
      end
      return _423_, not multi, 3
    else
      return nil, true, 2
    end
  end
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    local _426_
    if local_3f then
      _426_ = "local function %s(%s)"
    else
      _426_ = "%s = function(%s)"
    end
    compiler.emit(parent, string.format(_426_, fn_name, table.concat(arg_name_list, ", ")), ast)
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    if (utils["string?"](expr) and (index_2a < #ast)) then
      local _429_
      do
        local _428_ = f_metadata
        _428_["fnl/docstring"] = expr
        _429_ = _428_
      end
      return _429_, index_2a
    elseif (utils["table?"](expr) and (index_2a < #ast)) then
      local _430_
      do
        local tbl_11_auto = f_metadata
        for k, v in pairs(expr) do
          local _431_, _432_ = k, v
          if ((nil ~= _431_) and (nil ~= _432_)) then
            local k_12_auto = _431_
            local v_13_auto = _432_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
  ...
  SPECIALS.fn = function(ast, scope, parent)
    local f_scope
    do
      local _435_ = compiler["make-scope"](scope)
      do end (_435_)["vararg"] = false
      f_scope = _435_
    end
    local f_chunk = {}
  ...
  SPECIALS.lua = function(ast, _, parent)
    ...
    local _439_
    do
      local _438_ = utils["sym?"](ast[2])
      if (nil ~= _438_) then
        _439_ = tostring(_438_)
      else
        _439_ = _438_
      end
    end
    ...
    local _443_
    do
      local _442_ = utils["sym?"](ast[3])
      if (nil ~= _442_) then
        _443_ = tostring(_442_)
      else
        _443_ = _442_
      end
    end
  ...
  local function dot(ast, scope, parent)
    ...
    local len = #ast
    local _let_446_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local lhs = _let_446_[1]
    if (len == 2) then
      return tostring(lhs)
    else
      local indices = {}
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _let_447_ = compiler.compile1(index, scope, parent, {nval = 1})
          local index0 = _let_447_[1]
  ...
  local function disambiguate_3f(rootstr, parent)
    local function _456_()
      local _455_ = get_prev_line(parent)
      if (nil ~= _455_) then
        local prev_line = _455_
        return prev_line:match("%)$")
      else
        return nil
      end
    end
    return (rootstr:match("^{") or _456_())
  end
  SPECIALS.tset = function(ast, scope, parent)
    ...
    local keys = {}
    for i = 3, (#ast - 1) do
      local _let_458_ = compiler.compile1(ast[i], scope, parent, {nval = 1})
      local key = _let_458_[1]
  ...
  local function compile_until(condition, scope, chunk)
    if condition then
      local _let_467_ = compiler.compile1(condition, scope, chunk, {nval = 1})
      local condition_lua = _let_467_[1]
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    local _let_471_ = ast
    local _ = _let_471_[1]
    local _0 = _let_471_[2]
    local method_string = _let_471_[3]
    local call_string
  ...
  local function method_call(ast, scope, parent)
    ...
    for i = 4, #ast do
      ...
      subexprs = compiler.compile1(ast[i], scope, parent, {nval = _474_})
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local f_scope
    do
      local _479_ = compiler["make-scope"](scope)
      do end (_479_)["vararg"] = false
      _479_["hashfn"] = true
      f_scope = _479_
    end
    local f_chunk = {}
  ...
  local function maybe_short_circuit_protect(ast, i, name, _483_)
    local _arg_484_ = _483_
    local mac = _arg_484_["macros"]
  ...
  local function arithmetic_special(name, zero_arity, unary_prefix, ast, scope, parent)
    ...
    local _487_ = #operands
    if (_487_ == 0) then
      local _489_
      do
        local _488_ = zero_arity
        compiler.assert(_488_, "Expected more than 0 arguments", ast)
        _489_ = _488_
      end
      return utils.expr(_489_, "literal")
    elseif (_487_ == 1) then
  ...
  local function define_arithmetic_special(name, zero_arity, unary_prefix, _3flua_name)
    local _495_
    do
      local _492_ = (_3flua_name or name)
      local _493_ = zero_arity
      local _494_ = unary_prefix
      local function _496_(...)
        return arithmetic_special(_492_, _493_, _494_, ...)
      end
      _495_ = _496_
    end
    SPECIALS[name] = _495_
  ...
  local function define_bitop_special(name, zero_arity, unary_prefix, native)
    ...
    do
      local _503_ = native
      local _504_ = name
      local _505_ = zero_arity
      local _506_ = unary_prefix
      local function _508_(...)
        return bitop_special(_503_, _504_, _505_, _506_, ...)
      end
      _507_ = _508_
    end
    SPECIALS[name] = _507_
  ...
  local function native_comparator(op, _509_, scope, parent)
    local _arg_510_ = _509_
    local _ = _arg_510_[1]
    local lhs_ast = _arg_510_[2]
    local rhs_ast = _arg_510_[3]
    local _let_511_ = compiler.compile1(lhs_ast, scope, parent, {nval = 1})
    local lhs = _let_511_[1]
    local _let_512_ = compiler.compile1(rhs_ast, scope, parent, {nval = 1})
    local rhs = _let_512_[1]
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    local provided
    do
      local _520_ = (_3fopts or utils.root.options)
      if ((_G.type(_520_) == "table") and ((_520_)["compiler-env"] == "strict")) then
        provided = safe_compiler_env()
      elseif ((_G.type(_520_) == "table") and (nil ~= (_520_).compilerEnv)) then
        local compilerEnv = (_520_).compilerEnv
        provided = compilerEnv
      elseif ((_G.type(_520_) == "table") and (nil ~= (_520_)["compiler-env"])) then
        local compiler_env = (_520_)["compiler-env"]
  ...
  local _local_526_ = _527_(...)
  local dirsep = _local_526_[1]
  local pathsep = _local_526_[2]
  local pathmark = _local_526_[3]
  ...
  local function search_module(modulename, _3fpathstring)
    ...
    local function try_path(path)
      ...
      local _529_ = (io.open(filename) or io.open(filename2))
      if (nil ~= _529_) then
        local file = _529_
        file:close()
        return filename
    ...
    local function find_in_path(start, _3ftried_paths)
      local _531_ = fullpath:match(pattern, start)
      if (nil ~= _531_) then
        local path = _531_
        local _532_, _533_ = try_path(path)
        if (nil ~= _532_) then
          local filename = _532_
          return filename
        elseif ((_532_ == nil) and (nil ~= _533_)) then
          local error = _533_
          local function _535_()
            local _534_ = (_3ftried_paths or {})
            table.insert(_534_, error)
            return _534_
          end
          return find_in_path((start + #path + 1), _535_())
        else
          return nil
        end
      elseif true then
  ...
  local function make_searcher(_3foptions)
    local function _540_(module_name)
      local opts = utils.copy(utils.root.options)
      for k, v in pairs((_3foptions or {})) do
        opts[k] = v
      end
      opts["module-name"] = module_name
      local _541_, _542_ = search_module(module_name)
      if (nil ~= _541_) then
        local filename = _541_
        local _545_
        do
          local _543_ = filename
          local _544_ = opts
          local function _546_(...)
            return utils["fennel-module"].dofile(_543_, _544_, ...)
          end
          _545_ = _546_
        end
        return _545_, filename
      elseif ((_541_ == nil) and (nil ~= _542_)) then
        local error = _542_
        return error
      else
        return nil
  ...
  local function fennel_macro_searcher(module_name)
    local opts
    do
      local _548_ = utils.copy(utils.root.options)
      do end (_548_)["module-name"] = module_name
      _548_["env"] = "_COMPILER"
      _548_["requireAsInclude"] = false
      _548_["allowedGlobals"] = nil
      opts = _548_
    end
    local _549_ = search_module(module_name, utils["fennel-module"]["macro-path"])
    if (nil ~= _549_) then
      local filename = _549_
      local _550_
      if (opts["compiler-env"] == _G) then
        local _551_ = fennel_macro_searcher
        local _552_ = filename
        local _553_ = opts
        local function _555_(...)
          return dofile_with_searcher(_551_, _552_, _553_, ...)
        end
        _550_ = _555_
      else
        local _556_ = filename
        local _557_ = opts
        local function _559_(...)
          return utils["fennel-module"].dofile(_556_, _557_, ...)
        end
        _550_ = _559_
      end
      return _550_, filename
    else
      return nil
    end
  end
  local function lua_macro_searcher(module_name)
    local _562_ = search_module(module_name, package.path)
    if (nil ~= _562_) then
      local filename = _562_
      local code
      do
        ...
        code = close_handlers_8_auto(_G.xpcall(_564_, (package.loaded.fennel or debug).traceback))
      end
  ...
  local function search_macro_module(modname, n)
    local _566_ = macro_searchers[n]
    if (nil ~= _566_) then
      local f = _566_
      local _567_, _568_ = f(modname)
      if ((nil ~= _567_) and true) then
        local loader = _567_
        local _3ffilename = _568_
  ...
  local function _572_(modname)
    ...
    return (macro_loaded[modname] or metadata_only_fennel(modname) or _573_())
  end
  safe_require = _572_
  ...
  local function resolve_module_name(_574_, _scope, _parent, opts)
    local _arg_575_ = _574_
    local filename = _arg_575_["filename"]
    local second = _arg_575_[2]
...
if ("import-macros" == tostring(ast[1])) then
  ...
  local function include_path(ast, opts, path, mod, fennel_3f)
    ...
    do
      ...
      src = close_handlers_8_auto(_G.xpcall(_581_, (package.loaded.fennel or debug).traceback))
    end
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    local modexpr
    do
      local _584_, _585_ = pcall(resolve_module_name, ast, scope, parent, opts)
      if ((_584_ == true) and (nil ~= _585_)) then
        local modname = _585_
    ...
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      local function _589_()
        local _588_ = search_module(mod)
        if (nil ~= _588_) then
          local fennel_path = _588_
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _589_())
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"] or function(...)
  ...
  local function assert_compile(condition, msg, ast)
    if not condition then
      local _let_245_ = (utils.root.options or {})
      local source = _let_245_["source"]
      local unfriendly = _let_245_["unfriendly"]
  ...
  local function serialize_string(str)
    ...
    return string.gsub(string.gsub(string.format("%q", str), ".", serialize_subst), "[\128-\255]", _249_)
  end
  local function global_mangling(str)
    if utils["valid-lua-identifier?"](str) then
      ...
      return ("__fnl_global__" .. str:gsub("[^%w]", _250_))
    end
  end
  local function global_unmangling(identifier)
    local _252_ = string.match(identifier, "^__fnl_global__(.*)$")
    if (nil ~= _252_) then
      local rest = _252_
      local _253_
      ...
      _253_ = string.gsub(rest, "_[%da-f][%da-f]", _254_)
      return _253_
    elseif true then
      local _ = _252_
      return identifier
  ...
  local function local_mangling(str, scope, ast, _3ftemp_manglings)
    ...
    mangling = string.gsub(string.gsub(raw, "-", "_"), "[^%w_]", _258_)
  ...
  local function autogensym(base, scope)
    local _261_ = utils["multi-sym?"](base)
    if (nil ~= _261_) then
      local parts = _261_
    ...
    elseif true then
      ...
      return (scope.autogensyms[base] or _262_())
    else
      return nil
    end
  end
  ...
  local function flatten_chunk(sm, chunk, tab, depth)
    if chunk.leaf then
      ...
      do
        ...
        if (_275_ == true) then
          tab0 = "  "
        elseif (_275_ == false) then
          tab0 = ""
        elseif (_275_ == tab) then
          tab0 = tab
  ...
  local function make_metadata()
    local function _284_(self, tgt, key)
      if self[tgt] then
        return self[tgt][key]
      else
        return nil
    ...
    local function _286_(self, tgt, key, value)
      self[tgt] = (self[tgt] or {})
      do end (self[tgt])[key] = value
      return tgt
    end
    local function _287_(self, tgt, ...)
      local kv_len = select("#", ...)
      local kvs = {...}
      if ((kv_len % 2) ~= 0) then
    ...
    return setmetatable({}, {__index = {get = _284_, set = _286_, setall = _287_}, __mode = "k"})
  end
  ...
  local function handle_compile_opts(exprs, parent, opts, ast)
    ...
    if opts.target then
      ...
      emit(parent, string.format("%s = %s", opts.target, _295_()), ast)
    else
    end
  ...
  local function propagate_trace_info(_301_, _index, node)
    local _arg_302_ = _301_
    local filename = _arg_302_["filename"]
    local line = _arg_302_["line"]
    local bytestart = _arg_302_["bytestart"]
    local byteend = _arg_302_["byteend"]
  ...
  local function macroexpand_2a(ast, scope, _3fonce)
    ...
    if utils["list?"](ast) then
      _304_ = find_macro(ast, scope, utils["multi-sym?"](ast[1]))
    else
      _304_ = nil
    end
    if (_304_ == false) then
    ...
    elseif (nil ~= _304_) then
      ...
      ok, transformed = xpcall(_306_, debug.traceback)
      local function _308_()
        local _307_ = ast
        local function _309_(...)
          return propagate_trace_info(_307_, ...)
        end
        return _309_
      end
      utils["walk-tree"](transformed, _308_())
  ...
  local function compile_function_call(ast, scope, parent, opts, compile1, len)
    ...
    for i = 2, len do
      ...
      subexprs = compile1(ast[i], scope, parent, {nval = _315_})
  ...
  local function compile_varg(ast, scope, parent, opts)
    ...
    if scope.hashfn then
      ...
      _320_ = "unexpected vararg"
    end
    assert_compile(scope.vararg, _320_, ast)
  ...
  local function serialize_number(n)
    local _323_ = string.gsub(tostring(n), ",", ".")
    return _323_
  end
  local function compile_scalar(ast, _scope, parent, opts)
    local serialize
    do
      local _324_ = type(ast)
      if (_324_ == "nil") then
        serialize = tostring
      elseif (_324_ == "boolean") then
        serialize = tostring
      elseif (_324_ == "string") then
        serialize = serialize_string
      elseif (_324_ == "number") then
  ...
  local function compile_table(ast, scope, parent, opts, compile1)
    ...
    local function write_other_values(k)
      if ((type(k) ~= "number") or (math.floor(k) ~= k) or (k < 1) or (#ast < k)) then
        if ((type(k) == "string") and utils["valid-lua-identifier?"](k)) then
          ...
          local _let_326_ = compile1(k, scope, parent, {nval = 1})
          local compiled = _let_326_[1]
    ...
    do
      ...
      local function _332_(_330_)
        local _arg_331_ = _330_
        local k1 = _arg_331_[1]
        local k2 = _arg_331_[2]
        local _let_333_ = compile1(ast[k2], scope, parent, {nval = 1})
        local v = _let_333_[1]
        return string.format("%s = %s", k1, tostring(v))
      end
      utils.map(keys, _332_, buffer)
    end
    for i = 1, #ast do
  ...
  local function destructure(to, from, ast, scope, parent, opts)
    local opts0 = (opts or {})
    local _let_335_ = opts0
    local isvar = _let_335_["isvar"]
    local declaration = _let_335_["declaration"]
    local forceglobal = _let_335_["forceglobal"]
    local forceset = _let_335_["forceset"]
    local symtype = _let_335_["symtype"]
    ...
    local function destructure_kv_rest(s, v, left, excluded_keys, destructure1)
      ...
      exclude_str = table.concat(_348_, ", ")
    ...
    local function destructure_table(left, rightexprs, top_3f, destructure1)
      ...
      do
        local _350_
        if top_3f then
          _350_ = exprs1(compile1(from, scope, parent))
        else
          _350_ = exprs1(rightexprs)
        end
        if (_350_ == "") then
          right = "nil"
  ...
  local function traceback_frame(info)
    ...
    elseif (info.what == "C") then
      ...
      if (info.what == "Lua") then
        ...
        return string.format("  %s:%d: in function %s", info.short_src, info.currentline, _370_())
  ...
  local function traceback(_3fmsg, _3fstart)
    ...
    if ((msg:find("^Compile error") or msg:find("^Parse error")) and not utils["debug-on?"]("trace")) then
      ...
      while not done_3f do
        do
          local _374_ = debug.getinfo(level, "Sln")
          if (_374_ == nil) then
            done_3f = true
  ...
  local function do_quote(form, scope, parent, runtime_3f)
    ...
    elseif utils["list?"](form) then
      ...
      mapped = utils.kvmap(form, entry_transform(_382_, q))
      local filename
      if form.filename then
        filename = string.format("%q", form.filename)
      else
    ...
    elseif utils["sequence?"](form) then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s, sequence=%s})", mixed_concat(mapped, ", "), filename, _385_, "(getmetatable(sequence()))['sequence']")
    elseif (type(form) == "table") then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s})", mixed_concat(mapped, ", "), filename, _388_())
...
package.preload["fennel.friend"] = package.preload["fennel.friend"] or function(...)
  ...
  local function pinpoint(codeline, col, _3fendcol)
    if _3fendcol then
      return (string.rep(" ", col) .. string.rep("^", math.min((_3fendcol - col - -1), (utils.len(codeline) - col))))
    else
      return (string.rep("-", col) .. "^\n")
    end
  end
  local function friendly_msg(msg, _170_, source)
    local _arg_171_ = _170_
    local filename = _arg_171_["filename"]
    local line = _arg_171_["line"]
    local col = _arg_171_["col"]
    local endcol = _arg_171_["endcol"]
    ...
    if (ok and codeline and col) then
  ...
  local function assert_compile(condition, msg, ast, source)
    if not condition then
      local _let_174_ = utils["ast-source"](ast)
      local filename = _let_174_["filename"]
      local line = _let_174_["line"]
      local col = _let_174_["col"]
...
package.preload["fennel.parser"] = package.preload["fennel.parser"] or function(...)
  ...
  local function granulate(getchunk)
    ...
    local function _176_(parser_state)
      if not done_3f then
        if (index <= #c) then
          ...
          local _177_ = getchunk(parser_state)
          local function _178_()
            local char = _177_
            return (char ~= "")
          end
          if ((nil ~= _177_) and _178_()) then
            local char = _177_
            c = char
            index = 2
            return c:byte()
  ...
  local function whitespace_3f(b)
    return ((b == 32) or (function(_184_,_185_,_186_) return (_184_ <= _185_) and (_185_ <= _186_) end)(9,b,13))
  end
  ...
  local function char_starter_3f(b)
    return ((function(_188_,_189_,_190_) return (_188_ < _189_) and (_189_ < _190_) end)(1,b,127) or (function(_191_,_192_,_193_) return (_191_ < _192_) and (_192_ < _193_) end)(192,b,247))
  end
  local function parser_fn(getbyte, filename, _194_)
    local _arg_195_ = _194_
    local source = _arg_195_["source"]
    local unfriendly = _arg_195_["unfriendly"]
    local comments = _arg_195_["comments"]
    local stack = {}
    ...
    local function parse_stream()
      ...
      local function dispatch(v)
        local _203_ = stack[#stack]
        if (_203_ == nil) then
          retval, done_3f, whitespace_since_dispatch = v, true, false
          return nil
        elseif ((_G.type(_203_) == "table") and (nil ~= (_203_).prefix)) then
          local prefix = (_203_).prefix
          local source0
          do
            local _204_ = table.remove(stack)
            set_source_fields(_204_)
            source0 = _204_
          end
      ...
      local function badend()
        ...
        return parse_error(string.format("expected closing delimiter%s %s", _206_, string.char(unpack(accum))))
      end
      ...
      local function parse_comment(b, contents)
        if (b and (10 ~= b)) then
          local function _210_()
            local _209_ = contents
            table.insert(_209_, string.char(b))
            return _209_
          end
          return parse_comment(getb(), _210_())
      ...
      local function add_comment_at(comments0, index, node)
        local _213_ = (comments0)[index]
        if (nil ~= _213_) then
          local existing = _213_
      ...
      local function parse_string_loop(chars, b, state)
        ...
        do
          local _223_ = {state, b}
          if ((_G.type(_223_) == "table") and ((_223_)[1] == "base") and ((_223_)[2] == 92)) then
            state0 = "backslash"
          elseif ((_G.type(_223_) == "table") and ((_223_)[1] == "base") and ((_223_)[2] == 34)) then
            state0 = "done"
          elseif ((_G.type(_223_) == "table") and ((_223_)[1] == "backslash") and ((_223_)[2] == 10)) then
      ...
      local function parse_string()
        ...
        local _227_ = (rawget(_G, "loadstring") or load)(("return " .. formatted))
        if (nil ~= _227_) then
          local load_fn = _227_
      ...
      local function parse_number(rawstr)
        ...
        if rawstr:match("^%d") then
          ...
          local _233_ = tonumber(number_with_stripped_underscores)
          if (nil ~= _233_) then
            local x = _233_
            dispatch(x)
            return true
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _710_ = type(v)
    if (_710_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_710_ == "table") then
...
do
  ...
  local env
  do
    local _714_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_714_)["utils"] = utils
    _714_["fennel"] = mod
    env = _714_
  end
