package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_626_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _637_, _638_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_637_ == true) and (nil ~= _638_)) then
      local old = _638_
      local _
      ...
      return on_values({"ok"})
    elseif ((_637_ == false) and (nil ~= _638_)) then
      local msg = _638_
      ...
      elseif (specials["macro-loaded"])[module_name] then
        ...
        local function _643_()
          local _642_ = msg:gsub("\n.*", "")
          return _642_
        end
        return on_error("Runtime", _643_())
      end
    else
      return nil
    end
  ...
  local function run_command(read, on_error, f)
    local _646_, _647_, _648_ = pcall(read)
    if ((_646_ == true) and (_647_ == true) and (nil ~= _648_)) then
      local val = _648_
      return f(val)
    elseif (_646_ == false) then
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _652_ = type(subtbl)
        if (_652_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_652_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _655_, names)
          else
          end
        else
        end
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      do
        ...
        if ("function" == type(tgt)) then
          local _664_ = (compiler.metadata):get(tgt, "fnl/docstring")
          if (nil ~= _664_) then
            local docstr = _664_
  ...
  local function resolve(identifier, _671_, scope)
    local _arg_672_ = _671_
    local ___replLocals___ = _arg_672_["___replLocals___"]
    local env = _arg_672_
    local e
    local function _673_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _673_})
    local _674_, _675_ = pcall(compiler["compile-string"], tostring(identifier), {scope = scope})
    if ((_674_ == true) and (nil ~= _675_)) then
      local code = _675_
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _677_(_241)
      local _678_
      do
        local _679_ = utils["sym?"](_241)
        if (nil ~= _679_) then
          local _680_ = resolve(_679_, env, scope)
          if (nil ~= _680_) then
            _678_ = debug.getinfo(_680_)
          else
            _678_ = _680_
          end
        else
      ...
      if ((_G.type(_678_) == "table") and ((_678_).what == "Lua") and (nil ~= (_678_).source) and (nil ~= (_678_).linedefined) and (nil ~= (_678_).short_src)) then
        local source = (_678_).source
        local line = (_678_).linedefined
        local src = (_678_).short_src
        local fnlsrc
        do
          local t_683_ = compiler.sourcemap
          if (nil ~= t_683_) then
            t_683_ = (t_683_)[source]
          else
          end
          if (nil ~= t_683_) then
            t_683_ = (t_683_)[line]
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _693_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _693_) then
          local cmd_name = _693_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _695_ = commands[command_name]
      if (nil ~= _695_) then
        local command = _695_
      ...
      do
        ...
        do
          ...
          for k, v in pairs(readline.set_options({})) do
            local _700_, _701_ = k, v
            if ((nil ~= _700_) and (nil ~= _701_)) then
              local k_12_auto = _700_
              local v_13_auto = _701_
  ...
  local function repl(_3foptions)
    ...
    if opts.registerCompleter then
      local function _711_()
        local _709_ = env
        local _710_ = opts.scope
        local function _712_(...)
          return completer(_709_, _710_, ...)
        end
        return _712_
      end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if not_eof_3f then
          do
            local _716_, _717_ = nil, nil
            local function _719_()
              local _718_ = opts
              _718_["source"] = src_string
              return _718_
            end
            _716_, _717_ = pcall(compiler.compile, x, _719_())
            if ((_716_ == false) and (nil ~= _717_)) then
              local msg = _717_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_716_ == true) and (nil ~= _717_)) then
              local src = _717_
              local src0
              if save_locals_3f then
              ...
              local _721_, _722_ = pcall(specials["load-code"], src0, env)
              if ((_721_ == false) and (nil ~= _722_)) then
                local msg = _722_
                clear_stream()
                on_error("Lua Compile", msg, src0)
              elseif (true and (nil ~= _722_)) then
                local _ = _721_
                local chunk = _722_
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _737_ = type(v)
    if (_737_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_737_ == "table") then
...
do
  ...
  local env
  do
    local _741_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_741_)["utils"] = utils
    _741_["fennel"] = mod
    env = _741_
  end
