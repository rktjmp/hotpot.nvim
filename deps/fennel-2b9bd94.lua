package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_582_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _593_, _594_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_593_ == true) and (nil ~= _594_)) then
      local old = _594_
      local _
      ...
      return on_values({"ok"})
    elseif ((_593_ == false) and (nil ~= _594_)) then
      local msg = _594_
      if (specials["macro-loaded"])[module_name] then
        ...
        local function _599_()
          local _598_ = msg:gsub("\n.*", "")
          return _598_
        end
        return on_error("Runtime", _599_())
      end
    else
      return nil
    end
  ...
  local function run_command(read, on_error, f)
    local _602_, _603_, _604_ = pcall(read)
    if ((_602_ == true) and (_603_ == true) and (nil ~= _604_)) then
      local val = _604_
      return f(val)
    elseif (_602_ == false) then
  ...
  commands.reload = function(env, read, on_values, on_error)
    ...
    return run_command(read, on_error, _606_)
  end
  ...
  commands.complete = function(env, read, on_values, on_error, scope, chars)
    ...
    return run_command(read, on_error, _607_)
  end
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _608_ = type(subtbl)
        if (_608_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_608_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _611_, names)
          else
          end
        else
        end
  ...
  commands.apropos = function(_env, read, on_values, on_error, _scope)
    ...
    return run_command(read, on_error, _616_)
  end
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      do
        ...
        if ("function" == type(tgt)) then
          local _620_ = (compiler.metadata):get(tgt, "fnl/docstring")
          if (nil ~= _620_) then
            local docstr = _620_
  ...
  local function resolve(identifier, _627_, scope)
    local _arg_628_ = _627_
    local ___replLocals___ = _arg_628_["___replLocals___"]
    local env = _arg_628_
    local e
    local function _629_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _629_})
    local _630_, _631_ = pcall(compiler["compile-string"], tostring(identifier), {scope = scope})
    if ((_630_ == true) and (nil ~= _631_)) then
      local code = _631_
      local _632_ = specials["load-code"](code, e)()
      local function _633_()
        local x = _632_
        return (type(x) == "function")
      end
      if ((nil ~= _632_) and _633_()) then
        local x = _632_
        return x
      else
        return nil
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _636_(_241)
      local _637_
      do
        local _638_ = utils["sym?"](_241)
        if (nil ~= _638_) then
          local _639_ = resolve(_638_, env, scope)
          if (nil ~= _639_) then
            _637_ = debug.getinfo(_639_)
          else
            _637_ = _639_
          end
        else
      ...
      if ((_G.type(_637_) == "table") and (nil ~= (_637_).short_src) and (nil ~= (_637_).linedefined) and ((_637_).what == "Lua") and (nil ~= (_637_).source)) then
        local src = (_637_).short_src
        local line = (_637_).linedefined
        local source = (_637_).source
        local fnlsrc
        do
          local t_642_ = compiler.sourcemap
          if (nil ~= t_642_) then
            t_642_ = (t_642_)[source]
          else
          end
          if (nil ~= t_642_) then
            t_642_ = (t_642_)[line]
    ...
    return run_command(read, on_error, _636_)
  end
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _650_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _650_) then
          local cmd_name = _650_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _652_ = commands[command_name]
      if (nil ~= _652_) then
        local command = _652_
      ...
      do
        ...
        do
          ...
          for k, v in pairs(readline.set_options({})) do
            local _657_, _658_ = k, v
            if ((nil ~= _657_) and (nil ~= _658_)) then
              local k_12_auto = _657_
              local v_13_auto = _658_
  ...
  local function repl(_3foptions)
    ...
    local function _664_(parser_state)
      local c = byte_stream(parser_state)
      table.insert(chars, c)
      return c
    end
    read, reset = parser.parser(_664_)
    ...
    if opts.registerCompleter then
      local function _668_()
        local _666_ = env
        local _667_ = opts.scope
        local function _669_(...)
          return completer(_666_, _667_, ...)
        end
        return _669_
      end
      opts.registerCompleter(_668_())
    else
    end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if not_eof_3f then
          do
            local _673_, _674_ = nil, nil
            local function _676_()
              local _675_ = opts
              _675_["source"] = src_string
              return _675_
            end
            _673_, _674_ = pcall(compiler.compile, x, _676_())
            if ((_673_ == false) and (nil ~= _674_)) then
              local msg = _674_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_673_ == true) and (nil ~= _674_)) then
              local src = _674_
              local src0
              if save_locals_3f then
              ...
              local _678_, _679_ = pcall(specials["load-code"], src0, env)
              if ((_678_ == false) and (nil ~= _679_)) then
                local msg = _679_
                clear_stream()
                on_error("Lua Compile", msg, src0)
              elseif (true and (nil ~= _679_)) then
                local _ = _678_
                local chunk = _679_
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function load_code(code, _3fenv, _3ffilename)
    ...
    local _386_, _387_ = rawget(_G, "setfenv"), rawget(_G, "loadstring")
    if ((nil ~= _386_) and (nil ~= _387_)) then
      local setfenv = _386_
      local loadstring = _387_
  ...
  local function doc_2a(tgt, name)
    if not tgt then
      ...
      if ((type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function"))) then
        ...
        return string.format("(%s%s%s)\n  %s", name, _390_, arglist, docstring)
      else
  ...
  local function get_fn_name(ast, scope, fn_name, multi)
    if (fn_name and (fn_name[1] ~= "nil")) then
      local _407_
      if not multi then
        _407_ = compiler["declare-local"](fn_name, {}, scope, ast)
      else
        _407_ = (compiler["symbol-to-expression"](fn_name, scope))[1]
      end
      return _407_, not multi, 3
    else
      return nil, true, 2
    end
  end
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    local _410_
    if local_3f then
      _410_ = "local function %s(%s)"
    else
      _410_ = "%s = function(%s)"
    end
    compiler.emit(parent, string.format(_410_, fn_name, table.concat(arg_name_list, ", ")), ast)
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    if (utils["string?"](expr) and (index_2a < #ast)) then
      local _413_
      do
        local _412_ = f_metadata
        _412_["fnl/docstring"] = expr
        _413_ = _412_
      end
      return _413_, index_2a
    elseif (utils["table?"](expr) and (index_2a < #ast)) then
      local _414_
      do
        local tbl_11_auto = f_metadata
        for k, v in pairs(expr) do
          local _415_, _416_ = k, v
          if ((nil ~= _415_) and (nil ~= _416_)) then
            local k_12_auto = _415_
            local v_13_auto = _416_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
  ...
  SPECIALS.fn = function(ast, scope, parent)
    local f_scope
    do
      local _419_ = compiler["make-scope"](scope)
      do end (_419_)["vararg"] = false
      f_scope = _419_
    end
    local f_chunk = {}
  ...
  SPECIALS.lua = function(ast, _, parent)
    ...
    local _423_
    do
      local _422_ = utils["sym?"](ast[2])
      if (nil ~= _422_) then
        _423_ = tostring(_422_)
      else
        _423_ = _422_
      end
    end
    ...
    local _427_
    do
      local _426_ = utils["sym?"](ast[3])
      if (nil ~= _426_) then
        _427_ = tostring(_426_)
      else
        _427_ = _426_
      end
    end
  ...
  local function dot(ast, scope, parent)
    ...
    local len = #ast
    local _let_430_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local lhs = _let_430_[1]
    if (len == 2) then
      return tostring(lhs)
    else
      local indices = {}
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _let_431_ = compiler.compile1(index, scope, parent, {nval = 1})
          local index0 = _let_431_[1]
  ...
  local function disambiguate_3f(rootstr, parent)
    local function _440_()
      local _439_ = get_prev_line(parent)
      if (nil ~= _439_) then
        local prev_line = _439_
        return prev_line:match("%)$")
      else
        return nil
      end
    end
    return (rootstr:match("^{") or _440_())
  end
  SPECIALS.tset = function(ast, scope, parent)
    ...
    local keys = {}
    for i = 3, (#ast - 1) do
      local _let_442_ = compiler.compile1(ast[i], scope, parent, {nval = 1})
      local key = _let_442_[1]
  ...
  local function compile_until(condition, scope, chunk)
    if condition then
      local _let_451_ = compiler.compile1(condition, scope, chunk, {nval = 1})
      local condition_lua = _let_451_[1]
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    local _let_455_ = ast
    local _ = _let_455_[1]
    local _0 = _let_455_[2]
    local method_string = _let_455_[3]
    local call_string
  ...
  local function method_call(ast, scope, parent)
    ...
    local _let_457_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local target = _let_457_[1]
    local args = {}
    for i = 4, #ast do
      local subexprs
      local _458_
      ...
      subexprs = compiler.compile1(ast[i], scope, parent, {nval = _458_})
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local f_scope
    do
      local _463_ = compiler["make-scope"](scope)
      do end (_463_)["vararg"] = false
      _463_["hashfn"] = true
      f_scope = _463_
    end
    local f_chunk = {}
  ...
  local function maybe_short_circuit_protect(ast, i, name, _467_)
    local _arg_468_ = _467_
    local mac = _arg_468_["macros"]
  ...
  local function arithmetic_special(name, zero_arity, unary_prefix, ast, scope, parent)
    ...
    local _471_ = #operands
    if (_471_ == 0) then
      local _473_
      do
        local _472_ = zero_arity
        compiler.assert(_472_, "Expected more than 0 arguments", ast)
        _473_ = _472_
      end
      return utils.expr(_473_, "literal")
    elseif (_471_ == 1) then
  ...
  local function define_arithmetic_special(name, zero_arity, unary_prefix, _3flua_name)
    local _479_
    do
      local _476_ = (_3flua_name or name)
      local _477_ = zero_arity
      local _478_ = unary_prefix
      local function _480_(...)
        return arithmetic_special(_476_, _477_, _478_, ...)
      end
      _479_ = _480_
    end
    SPECIALS[name] = _479_
  ...
  local function define_bitop_special(name, zero_arity, unary_prefix, native)
    ...
    do
      ...
      local _490_ = unary_prefix
      local function _492_(...)
        return bitop_special(_487_, _488_, _489_, _490_, ...)
      end
      _491_ = _492_
    end
    SPECIALS[name] = _491_
  ...
  local function native_comparator(op, _493_, scope, parent)
    local _arg_494_ = _493_
    local _ = _arg_494_[1]
    local lhs_ast = _arg_494_[2]
    local rhs_ast = _arg_494_[3]
    local _let_495_ = compiler.compile1(lhs_ast, scope, parent, {nval = 1})
    local lhs = _let_495_[1]
    local _let_496_ = compiler.compile1(rhs_ast, scope, parent, {nval = 1})
    local rhs = _let_496_[1]
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    local provided
    do
      local _504_ = (_3fopts or utils.root.options)
      if ((_G.type(_504_) == "table") and ((_504_)["compiler-env"] == "strict")) then
        provided = safe_compiler_env()
      elseif ((_G.type(_504_) == "table") and (nil ~= (_504_).compilerEnv)) then
        local compilerEnv = (_504_).compilerEnv
        provided = compilerEnv
      elseif ((_G.type(_504_) == "table") and (nil ~= (_504_)["compiler-env"])) then
        local compiler_env = (_504_)["compiler-env"]
  ...
  local _local_510_ = _511_(...)
  local dirsep = _local_510_[1]
  local pathsep = _local_510_[2]
  local pathmark = _local_510_[3]
  ...
  local function search_module(modulename, _3fpathstring)
    ...
    local function try_path(path)
      ...
      local _513_ = (io.open(filename) or io.open(filename2))
      if (nil ~= _513_) then
        local file = _513_
        file:close()
        return filename
    ...
    local function find_in_path(start, _3ftried_paths)
      local _515_ = fullpath:match(pattern, start)
      if (nil ~= _515_) then
        local path = _515_
        local _516_, _517_ = try_path(path)
        if (nil ~= _516_) then
          local filename = _516_
          return filename
        elseif ((_516_ == nil) and (nil ~= _517_)) then
          local error = _517_
          local function _519_()
            local _518_ = (_3ftried_paths or {})
            table.insert(_518_, error)
            return _518_
          end
          return find_in_path((start + #path + 1), _519_())
        else
          return nil
        end
      elseif true then
  ...
  local function make_searcher(_3foptions)
    local function _524_(module_name)
      local opts = utils.copy(utils.root.options)
      for k, v in pairs((_3foptions or {})) do
        opts[k] = v
      end
      opts["module-name"] = module_name
      local _525_, _526_ = search_module(module_name)
      if (nil ~= _525_) then
        local filename = _525_
        local _529_
        do
          local _527_ = filename
          local _528_ = opts
          local function _530_(...)
            return utils["fennel-module"].dofile(_527_, _528_, ...)
          end
          _529_ = _530_
        end
        return _529_, filename
      elseif ((_525_ == nil) and (nil ~= _526_)) then
        local error = _526_
        return error
      else
        return nil
  ...
  local function fennel_macro_searcher(module_name)
    local opts
    do
      local _532_ = utils.copy(utils.root.options)
      do end (_532_)["module-name"] = module_name
      _532_["env"] = "_COMPILER"
      _532_["requireAsInclude"] = false
      _532_["allowedGlobals"] = nil
      opts = _532_
    end
    local _533_ = search_module(module_name, utils["fennel-module"]["macro-path"])
    if (nil ~= _533_) then
      local filename = _533_
      local _534_
      if (opts["compiler-env"] == _G) then
        local _535_ = fennel_macro_searcher
        local _536_ = filename
        local _537_ = opts
        local function _539_(...)
          return dofile_with_searcher(_535_, _536_, _537_, ...)
        end
        _534_ = _539_
      else
        local _540_ = filename
        local _541_ = opts
        local function _543_(...)
          return utils["fennel-module"].dofile(_540_, _541_, ...)
        end
        _534_ = _543_
      end
  ...
  local function lua_macro_searcher(module_name)
    local _546_ = search_module(module_name, package.path)
    if (nil ~= _546_) then
      local filename = _546_
      local code
      do
        ...
        code = close_handlers_8_auto(_G.xpcall(_548_, (package.loaded.fennel or debug).traceback))
      end
  ...
  local function search_macro_module(modname, n)
    local _550_ = macro_searchers[n]
    if (nil ~= _550_) then
      local f = _550_
      local _551_, _552_ = f(modname)
      if ((nil ~= _551_) and true) then
        local loader = _551_
        local _3ffilename = _552_
  ...
  local function _556_(modname)
    ...
    return (macro_loaded[modname] or metadata_only_fennel(modname) or _557_())
  end
  safe_require = _556_
  ...
  local function resolve_module_name(_558_, _scope, _parent, opts)
    local _arg_559_ = _558_
    local filename = _arg_559_["filename"]
    local second = _arg_559_[2]
...
if ("import-macros" == tostring(ast[1])) then
  ...
  local function include_path(ast, opts, path, mod, fennel_3f)
    ...
    do
      ...
      src = close_handlers_8_auto(_G.xpcall(_565_, (package.loaded.fennel or debug).traceback))
    end
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    local modexpr
    do
      local _568_, _569_ = pcall(resolve_module_name, ast, scope, parent, opts)
      if ((_568_ == true) and (nil ~= _569_)) then
        local modname = _569_
    ...
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      local function _573_()
        local _572_ = search_module(mod)
        if (nil ~= _572_) then
          local fennel_path = _572_
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _573_())
...
local function eval(str, options, ...)
  ...
  loader = specials["load-code"](lua_source, env, _693_(...))
  opts.filename = nil
  return loader(...)
end
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _694_ = type(v)
    if (_694_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_694_ == "table") then
...
do
  ...
  package.preload[module_name] = _697_
  _ = nil
  local env
  do
    local _698_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_698_)["utils"] = utils
    _698_["fennel"] = mod
    env = _698_
  end
