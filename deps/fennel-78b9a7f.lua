package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_587_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _598_, _599_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_598_ == true) and (nil ~= _599_)) then
      local old = _599_
      local _
      ...
      return on_values({"ok"})
    elseif ((_598_ == false) and (nil ~= _599_)) then
      local msg = _599_
      if (specials["macro-loaded"])[module_name] then
        ...
        local function _604_()
          local _603_ = msg:gsub("\n.*", "")
          return _603_
        end
        return on_error("Runtime", _604_())
      end
    else
      return nil
    end
  ...
  local function run_command(read, on_error, f)
    local _607_, _608_, _609_ = pcall(read)
    if ((_607_ == true) and (_608_ == true) and (nil ~= _609_)) then
      local val = _609_
      return f(val)
    elseif (_607_ == false) then
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _613_ = type(subtbl)
        if (_613_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_613_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _616_, names)
          else
          end
        else
        end
  ...
  commands.apropos = function(_env, read, on_values, on_error, _scope)
    ...
    return run_command(read, on_error, _621_)
  end
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      do
        ...
        if ("function" == type(tgt)) then
          local _625_ = (compiler.metadata):get(tgt, "fnl/docstring")
          if (nil ~= _625_) then
            local docstr = _625_
  ...
  local function resolve(identifier, _632_, scope)
    local _arg_633_ = _632_
    local ___replLocals___ = _arg_633_["___replLocals___"]
    local env = _arg_633_
    local e
    local function _634_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _634_})
    local _635_, _636_ = pcall(compiler["compile-string"], tostring(identifier), {scope = scope})
    if ((_635_ == true) and (nil ~= _636_)) then
      local code = _636_
      local _637_ = specials["load-code"](code, e)()
      local function _638_()
        local x = _637_
        return (type(x) == "function")
      end
      if ((nil ~= _637_) and _638_()) then
        local x = _637_
        return x
      else
        return nil
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _641_(_241)
      local _642_
      do
        local _643_ = utils["sym?"](_241)
        if (nil ~= _643_) then
          local _644_ = resolve(_643_, env, scope)
          if (nil ~= _644_) then
            _642_ = debug.getinfo(_644_)
          else
            _642_ = _644_
          end
        else
      ...
      if ((_G.type(_642_) == "table") and (nil ~= (_642_).source) and (nil ~= (_642_).linedefined) and ((_642_).what == "Lua") and (nil ~= (_642_).short_src)) then
        local source = (_642_).source
        local line = (_642_).linedefined
        local src = (_642_).short_src
        local fnlsrc
        do
          local t_647_ = compiler.sourcemap
          if (nil ~= t_647_) then
            t_647_ = (t_647_)[source]
          else
          end
          if (nil ~= t_647_) then
            t_647_ = (t_647_)[line]
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _655_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _655_) then
          local cmd_name = _655_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _657_ = commands[command_name]
      if (nil ~= _657_) then
        local command = _657_
      ...
      do
        ...
        do
          ...
          for k, v in pairs(readline.set_options({})) do
            local _662_, _663_ = k, v
            if ((nil ~= _662_) and (nil ~= _663_)) then
              local k_12_auto = _662_
              local v_13_auto = _663_
  ...
  local function repl(_3foptions)
    ...
    local function _669_(parser_state)
      local c = byte_stream(parser_state)
      table.insert(chars, c)
      return c
    end
    read, reset = parser.parser(_669_)
    ...
    if opts.registerCompleter then
      local function _673_()
        local _671_ = env
        local _672_ = opts.scope
        local function _674_(...)
          return completer(_671_, _672_, ...)
        end
        return _674_
      end
      opts.registerCompleter(_673_())
    else
    end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if not_eof_3f then
          do
            local _678_, _679_ = nil, nil
            local function _681_()
              local _680_ = opts
              _680_["source"] = src_string
              return _680_
            end
            _678_, _679_ = pcall(compiler.compile, x, _681_())
            if ((_678_ == false) and (nil ~= _679_)) then
              local msg = _679_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_678_ == true) and (nil ~= _679_)) then
              local src = _679_
              local src0
              if save_locals_3f then
              ...
              local _683_, _684_ = pcall(specials["load-code"], src0, env)
              if ((_683_ == false) and (nil ~= _684_)) then
                local msg = _684_
                clear_stream()
                on_error("Lua Compile", msg, src0)
              elseif (true and (nil ~= _684_)) then
                local _ = _683_
                local chunk = _684_
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function wrap_env(env)
    ...
    local function _381_(_, key, value)
      if utils["string?"](key) then
    ...
    local function _383_()
      local function putenv(k, v)
        ...
        if utils["string?"](k) then
          _384_ = compiler["global-unmangling"](k)
        else
          _384_ = k
        end
        return _384_, v
    ...
    return setmetatable({}, {__index = _379_, __newindex = _381_, __pairs = _383_})
  end
  local function current_global_names(_3fenv)
    local mt
    do
      local _386_ = getmetatable(_3fenv)
      if ((_G.type(_386_) == "table") and (nil ~= (_386_).__pairs)) then
        local mtpairs = (_386_).__pairs
        local tbl_11_auto = {}
        for k, v in mtpairs(_3fenv) do
          local _387_, _388_ = k, v
          if ((nil ~= _387_) and (nil ~= _388_)) then
            local k_12_auto = _387_
            local v_13_auto = _388_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
        mt = tbl_11_auto
      elseif (_386_ == nil) then
        mt = (_3fenv or _G)
      else
        mt = nil
      end
  ...
  local function load_code(code, _3fenv, _3ffilename)
    ...
    local _391_, _392_ = rawget(_G, "setfenv"), rawget(_G, "loadstring")
    if ((nil ~= _391_) and (nil ~= _392_)) then
      local setfenv = _391_
      local loadstring = _392_
  ...
  local function doc_2a(tgt, name)
    if not tgt then
      ...
      if ((type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function"))) then
        ...
        return string.format("(%s%s%s)\n  %s", name, _395_, arglist, docstring)
      else
  ...
  local function set_fn_metadata(arg_list, docstring, parent, fn_name)
    if utils.root.options.useMetadata then
      local args
      local function _409_(_241)
        return ("\"%s\""):format(deep_tostring(_241))
      end
      args = utils.map(arg_list, _409_)
  ...
  local function get_fn_name(ast, scope, fn_name, multi)
    if (fn_name and (fn_name[1] ~= "nil")) then
      local _412_
      if not multi then
        _412_ = compiler["declare-local"](fn_name, {}, scope, ast)
      else
        _412_ = (compiler["symbol-to-expression"](fn_name, scope))[1]
      end
      return _412_, not multi, 3
    else
      return nil, true, 2
    end
  end
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    local _415_
    if local_3f then
      _415_ = "local function %s(%s)"
    else
      _415_ = "%s = function(%s)"
    end
    compiler.emit(parent, string.format(_415_, fn_name, table.concat(arg_name_list, ", ")), ast)
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    if (utils["string?"](expr) and (index_2a < #ast)) then
      local _418_
      do
        local _417_ = f_metadata
        _417_["fnl/docstring"] = expr
        _418_ = _417_
      end
      return _418_, index_2a
  ...
  SPECIALS.fn = function(ast, scope, parent)
    local f_scope
    do
      local _424_ = compiler["make-scope"](scope)
      do end (_424_)["vararg"] = false
      f_scope = _424_
    end
    local f_chunk = {}
  ...
  SPECIALS.lua = function(ast, _, parent)
    ...
    local _428_
    do
      local _427_ = utils["sym?"](ast[2])
      if (nil ~= _427_) then
        _428_ = tostring(_427_)
      else
        _428_ = _427_
      end
    end
    ...
    local _432_
    do
      local _431_ = utils["sym?"](ast[3])
      if (nil ~= _431_) then
        _432_ = tostring(_431_)
      else
        _432_ = _431_
      end
    end
  ...
  local function dot(ast, scope, parent)
    ...
    local len = #ast
    local _let_435_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local lhs = _let_435_[1]
    if (len == 2) then
      return tostring(lhs)
    else
      local indices = {}
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _let_436_ = compiler.compile1(index, scope, parent, {nval = 1})
          local index0 = _let_436_[1]
  ...
  local function disambiguate_3f(rootstr, parent)
    local function _445_()
      local _444_ = get_prev_line(parent)
      if (nil ~= _444_) then
        local prev_line = _444_
        return prev_line:match("%)$")
      else
        return nil
      end
    end
    return (rootstr:match("^{") or _445_())
  end
  SPECIALS.tset = function(ast, scope, parent)
    ...
    local keys = {}
    for i = 3, (#ast - 1) do
      local _let_447_ = compiler.compile1(ast[i], scope, parent, {nval = 1})
      local key = _let_447_[1]
  ...
  local function compile_until(condition, scope, chunk)
    if condition then
      local _let_456_ = compiler.compile1(condition, scope, chunk, {nval = 1})
      local condition_lua = _let_456_[1]
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    local _let_460_ = ast
    local _ = _let_460_[1]
    local _0 = _let_460_[2]
    local method_string = _let_460_[3]
    local call_string
  ...
  local function method_call(ast, scope, parent)
    ...
    local _let_462_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local target = _let_462_[1]
    local args = {}
    for i = 4, #ast do
      local subexprs
      local _463_
      ...
      subexprs = compiler.compile1(ast[i], scope, parent, {nval = _463_})
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local f_scope
    do
      local _468_ = compiler["make-scope"](scope)
      do end (_468_)["vararg"] = false
      _468_["hashfn"] = true
      f_scope = _468_
    end
    local f_chunk = {}
  ...
  local function maybe_short_circuit_protect(ast, i, name, _472_)
    local _arg_473_ = _472_
    local mac = _arg_473_["macros"]
  ...
  local function arithmetic_special(name, zero_arity, unary_prefix, ast, scope, parent)
    ...
    local _476_ = #operands
    if (_476_ == 0) then
      local _478_
      do
        local _477_ = zero_arity
        compiler.assert(_477_, "Expected more than 0 arguments", ast)
        _478_ = _477_
      end
      return utils.expr(_478_, "literal")
    elseif (_476_ == 1) then
  ...
  local function define_arithmetic_special(name, zero_arity, unary_prefix, _3flua_name)
    local _484_
    do
      local _481_ = (_3flua_name or name)
      local _482_ = zero_arity
      local _483_ = unary_prefix
      local function _485_(...)
        return arithmetic_special(_481_, _482_, _483_, ...)
      end
      _484_ = _485_
    end
    SPECIALS[name] = _484_
  ...
  local function bitop_special(native_name, lib_name, zero_arity, unary_prefix, ast, scope, parent)
    if (#ast == 1) then
      ...
      for i = 2, len do
        ...
        subexprs = compiler.compile1(ast[i], scope, parent, {nval = _486_})
        utils.map(subexprs, tostring, operands)
      end
      if (#operands == 1) then
  ...
  local function define_bitop_special(name, zero_arity, unary_prefix, native)
    ...
    do
      ...
      local function _497_(...)
        return bitop_special(_492_, _493_, _494_, _495_, ...)
      end
      _496_ = _497_
    end
    SPECIALS[name] = _496_
  ...
  local function native_comparator(op, _498_, scope, parent)
    local _arg_499_ = _498_
    local _ = _arg_499_[1]
    local lhs_ast = _arg_499_[2]
    local rhs_ast = _arg_499_[3]
    local _let_500_ = compiler.compile1(lhs_ast, scope, parent, {nval = 1})
    local lhs = _let_500_[1]
    local _let_501_ = compiler.compile1(rhs_ast, scope, parent, {nval = 1})
    local rhs = _let_501_[1]
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    local provided
    do
      local _509_ = (_3fopts or utils.root.options)
      if ((_G.type(_509_) == "table") and ((_509_)["compiler-env"] == "strict")) then
        provided = safe_compiler_env()
      elseif ((_G.type(_509_) == "table") and (nil ~= (_509_).compilerEnv)) then
        local compilerEnv = (_509_).compilerEnv
        provided = compilerEnv
      elseif ((_G.type(_509_) == "table") and (nil ~= (_509_)["compiler-env"])) then
        local compiler_env = (_509_)["compiler-env"]
  ...
  local _local_515_ = _516_(...)
  local dirsep = _local_515_[1]
  local pathsep = _local_515_[2]
  local pathmark = _local_515_[3]
  ...
  local function search_module(modulename, _3fpathstring)
    ...
    local function try_path(path)
      ...
      local _518_ = (io.open(filename) or io.open(filename2))
      if (nil ~= _518_) then
        local file = _518_
        file:close()
        return filename
    ...
    local function find_in_path(start, _3ftried_paths)
      local _520_ = fullpath:match(pattern, start)
      if (nil ~= _520_) then
        local path = _520_
        local _521_, _522_ = try_path(path)
        if (nil ~= _521_) then
          local filename = _521_
          return filename
        elseif ((_521_ == nil) and (nil ~= _522_)) then
          local error = _522_
          local function _524_()
            local _523_ = (_3ftried_paths or {})
            table.insert(_523_, error)
            return _523_
          end
          return find_in_path((start + #path + 1), _524_())
        else
          return nil
        end
      elseif true then
  ...
  local function make_searcher(_3foptions)
    local function _529_(module_name)
      local opts = utils.copy(utils.root.options)
      for k, v in pairs((_3foptions or {})) do
        opts[k] = v
      end
      opts["module-name"] = module_name
      local _530_, _531_ = search_module(module_name)
      if (nil ~= _530_) then
        local filename = _530_
        local _534_
        do
          local _532_ = filename
          local _533_ = opts
          local function _535_(...)
            return utils["fennel-module"].dofile(_532_, _533_, ...)
          end
          _534_ = _535_
        end
        return _534_, filename
      elseif ((_530_ == nil) and (nil ~= _531_)) then
        local error = _531_
        return error
      else
        return nil
  ...
  local function fennel_macro_searcher(module_name)
    local opts
    do
      local _537_ = utils.copy(utils.root.options)
      do end (_537_)["module-name"] = module_name
      _537_["env"] = "_COMPILER"
      _537_["requireAsInclude"] = false
      _537_["allowedGlobals"] = nil
      opts = _537_
    end
    local _538_ = search_module(module_name, utils["fennel-module"]["macro-path"])
    if (nil ~= _538_) then
      local filename = _538_
      local _539_
      if (opts["compiler-env"] == _G) then
        local _540_ = fennel_macro_searcher
        local _541_ = filename
        local _542_ = opts
        local function _544_(...)
          return dofile_with_searcher(_540_, _541_, _542_, ...)
        end
        _539_ = _544_
      else
        local _545_ = filename
        local _546_ = opts
        local function _548_(...)
          return utils["fennel-module"].dofile(_545_, _546_, ...)
        end
        _539_ = _548_
      end
      return _539_, filename
    else
      return nil
    end
  end
  local function lua_macro_searcher(module_name)
    local _551_ = search_module(module_name, package.path)
    if (nil ~= _551_) then
      local filename = _551_
      local code
      do
        ...
        code = close_handlers_8_auto(_G.xpcall(_553_, (package.loaded.fennel or debug).traceback))
      end
  ...
  local function search_macro_module(modname, n)
    local _555_ = macro_searchers[n]
    if (nil ~= _555_) then
      local f = _555_
      local _556_, _557_ = f(modname)
      if ((nil ~= _556_) and true) then
        local loader = _556_
        local _3ffilename = _557_
  ...
  local function _561_(modname)
    ...
    return (macro_loaded[modname] or metadata_only_fennel(modname) or _562_())
  end
  safe_require = _561_
  ...
  local function resolve_module_name(_563_, _scope, _parent, opts)
    local _arg_564_ = _563_
    local filename = _arg_564_["filename"]
    local second = _arg_564_[2]
...
if ("import-macros" == tostring(ast[1])) then
  ...
  local function include_path(ast, opts, path, mod, fennel_3f)
    ...
    do
      ...
      src = close_handlers_8_auto(_G.xpcall(_570_, (package.loaded.fennel or debug).traceback))
    end
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    local modexpr
    do
      local _573_, _574_ = pcall(resolve_module_name, ast, scope, parent, opts)
      if ((_573_ == true) and (nil ~= _574_)) then
        local modname = _574_
    ...
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      local function _578_()
        local _577_ = search_module(mod)
        if (nil ~= _577_) then
          local fennel_path = _577_
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _578_())
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"] or function(...)
  ...
  local function assert_compile(condition, msg, ast)
    if not condition then
      local _let_234_ = (utils.root.options or {})
      local source = _let_234_["source"]
      local unfriendly = _let_234_["unfriendly"]
  ...
  local function serialize_string(str)
    ...
    return string.gsub(string.gsub(string.format("%q", str), ".", serialize_subst), "[\128-\255]", _238_)
  end
  local function global_mangling(str)
    if utils["valid-lua-identifier?"](str) then
      ...
      return ("__fnl_global__" .. str:gsub("[^%w]", _239_))
    end
  end
  local function global_unmangling(identifier)
    local _241_ = string.match(identifier, "^__fnl_global__(.*)$")
    if (nil ~= _241_) then
      local rest = _241_
      local _242_
      ...
      _242_ = string.gsub(rest, "_[%da-f][%da-f]", _243_)
      return _242_
    elseif true then
      local _ = _241_
      return identifier
  ...
  local function local_mangling(str, scope, ast, _3ftemp_manglings)
    ...
    mangling = string.gsub(string.gsub(raw, "-", "_"), "[^%w_]", _247_)
  ...
  local function autogensym(base, scope)
    local _250_ = utils["multi-sym?"](base)
    if (nil ~= _250_) then
      local parts = _250_
    ...
    elseif true then
      ...
      return (scope.autogensyms[base] or _251_())
    else
      return nil
    end
  end
  ...
  local function make_metadata()
    local function _273_(self, tgt, key)
      if self[tgt] then
        return self[tgt][key]
      else
        return nil
    ...
    local function _275_(self, tgt, key, value)
      self[tgt] = (self[tgt] or {})
      do end (self[tgt])[key] = value
      return tgt
    end
    local function _276_(self, tgt, ...)
      local kv_len = select("#", ...)
      local kvs = {...}
      if ((kv_len % 2) ~= 0) then
    ...
    return setmetatable({}, {__index = {get = _273_, set = _275_, setall = _276_}, __mode = "k"})
  end
  ...
  local function handle_compile_opts(exprs, parent, opts, ast)
    ...
    if opts.target then
      ...
      emit(parent, string.format("%s = %s", opts.target, _284_()), ast)
    else
    end
  ...
  local function propagate_trace_info(_290_, _index, node)
    local _arg_291_ = _290_
    local filename = _arg_291_["filename"]
    local line = _arg_291_["line"]
    local bytestart = _arg_291_["bytestart"]
    local byteend = _arg_291_["byteend"]
  ...
  local function macroexpand_2a(ast, scope, _3fonce)
    ...
    if utils["list?"](ast) then
      _293_ = find_macro(ast, scope, utils["multi-sym?"](ast[1]))
    else
      _293_ = nil
    end
    if (_293_ == false) then
    ...
    elseif (nil ~= _293_) then
      ...
      ok, transformed = xpcall(_295_, debug.traceback)
      local function _297_()
        local _296_ = ast
        local function _298_(...)
          return propagate_trace_info(_296_, ...)
        end
        return _298_
      end
      utils["walk-tree"](transformed, _297_())
  ...
  local function compile_function_call(ast, scope, parent, opts, compile1, len)
    ...
    for i = 2, len do
      ...
      subexprs = compile1(ast[i], scope, parent, {nval = _304_})
  ...
  local function compile_varg(ast, scope, parent, opts)
    ...
    if scope.hashfn then
      ...
      _309_ = "unexpected vararg"
    end
    assert_compile(scope.vararg, _309_, ast)
  ...
  local function serialize_number(n)
    local _312_ = string.gsub(tostring(n), ",", ".")
    return _312_
  end
  local function compile_scalar(ast, _scope, parent, opts)
    local serialize
    do
      local _313_ = type(ast)
      if (_313_ == "nil") then
        serialize = tostring
      elseif (_313_ == "boolean") then
        serialize = tostring
      elseif (_313_ == "string") then
        serialize = serialize_string
      elseif (_313_ == "number") then
  ...
  local function compile_table(ast, scope, parent, opts, compile1)
    ...
    local function write_other_values(k)
      if ((type(k) ~= "number") or (math.floor(k) ~= k) or (k < 1) or (#ast < k)) then
        if ((type(k) == "string") and utils["valid-lua-identifier?"](k)) then
          ...
          local _let_315_ = compile1(k, scope, parent, {nval = 1})
          local compiled = _let_315_[1]
    ...
    do
      ...
      local function _321_(_319_)
        local _arg_320_ = _319_
        local k1 = _arg_320_[1]
        local k2 = _arg_320_[2]
        local _let_322_ = compile1(ast[k2], scope, parent, {nval = 1})
        local v = _let_322_[1]
        return string.format("%s = %s", k1, tostring(v))
      end
      utils.map(keys, _321_, buffer)
    end
    for i = 1, #ast do
  ...
  local function destructure(to, from, ast, scope, parent, opts)
    local opts0 = (opts or {})
    local _let_324_ = opts0
    local isvar = _let_324_["isvar"]
    local declaration = _let_324_["declaration"]
    local forceglobal = _let_324_["forceglobal"]
    local forceset = _let_324_["forceset"]
    local symtype = _let_324_["symtype"]
    ...
    local function destructure_kv_rest(s, v, left, excluded_keys, destructure1)
      ...
      exclude_str = table.concat(_337_, ", ")
    ...
    local function destructure_table(left, rightexprs, top_3f, destructure1)
      ...
      do
        local _339_
        if top_3f then
          _339_ = exprs1(compile1(from, scope, parent))
        else
          _339_ = exprs1(rightexprs)
        end
        if (_339_ == "") then
          right = "nil"
  ...
  local function traceback_frame(info)
    ...
    elseif (info.what == "C") then
      ...
      if (info.what == "Lua") then
        ...
        return string.format("  %s:%d: in function %s", info.short_src, info.currentline, _359_())
  ...
  local function traceback(_3fmsg, _3fstart)
    ...
    if ((msg:find("^Compile error") or msg:find("^Parse error")) and not utils["debug-on?"]("trace")) then
      ...
      while not done_3f do
        do
          local _363_ = debug.getinfo(level, "Sln")
          if (_363_ == nil) then
            done_3f = true
  ...
  local function do_quote(form, scope, parent, runtime_3f)
    ...
    elseif utils["list?"](form) then
      ...
      mapped = utils.kvmap(form, entry_transform(_371_, q))
      local filename
      if form.filename then
        filename = string.format("%q", form.filename)
      else
    ...
    elseif utils["sequence?"](form) then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s, sequence=%s})", mixed_concat(mapped, ", "), filename, _374_, "(getmetatable(sequence()))['sequence']")
    elseif (type(form) == "table") then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s})", mixed_concat(mapped, ", "), filename, _377_())
...
package.preload["fennel.friend"] = package.preload["fennel.friend"] or function(...)
  ...
  local function friendly_msg(msg, _165_, source)
    local _arg_166_ = _165_
    local filename = _arg_166_["filename"]
    local line = _arg_166_["line"]
    local bytestart = _arg_166_["bytestart"]
    local byteend = _arg_166_["byteend"]
  ...
  local function assert_compile(condition, msg, ast, source)
    if not condition then
      local _let_171_ = utils["ast-source"](ast)
      local filename = _let_171_["filename"]
      local line = _let_171_["line"]
...
package.preload["fennel.parser"] = package.preload["fennel.parser"] or function(...)
  ...
  local function granulate(getchunk)
    ...
    local function _173_(parser_state)
      if not done_3f then
        if (index <= #c) then
          ...
          local _174_ = getchunk(parser_state)
          local function _175_()
            local char = _174_
            return (char ~= "")
          end
          if ((nil ~= _174_) and _175_()) then
            local char = _174_
            c = char
            index = 2
            return c:byte()
  ...
  local function whitespace_3f(b)
    return ((b == 32) or (function(_181_,_182_,_183_) return (_181_ <= _182_) and (_182_ <= _183_) end)(9,b,13))
  end
  ...
  local function parser_fn(getbyte, filename, _185_)
    local _arg_186_ = _185_
    local source = _arg_186_["source"]
    local unfriendly = _arg_186_["unfriendly"]
    local comments = _arg_186_["comments"]
    local stack = {}
    local line = 1
    local byteindex = 0
    local lastb = nil
    ...
    local function parse_stream()
      ...
      local function dispatch(v)
        local _192_ = stack[#stack]
        if (_192_ == nil) then
          retval, done_3f, whitespace_since_dispatch = v, true, false
          return nil
        elseif ((_G.type(_192_) == "table") and (nil ~= (_192_).prefix)) then
          local prefix = (_192_).prefix
          local source0
          do
            local _193_ = table.remove(stack)
            do end (_193_)["byteend"] = byteindex
            source0 = _193_
          end
      ...
      local function badend()
        ...
        return parse_error(string.format("expected closing delimiter%s %s", _195_, string.char(unpack(accum))))
      end
      ...
      local function parse_comment(b, contents)
        if (b and (10 ~= b)) then
          local function _199_()
            local _198_ = contents
            table.insert(_198_, string.char(b))
            return _198_
          end
          return parse_comment(getb(), _199_())
      ...
      local function add_comment_at(comments0, index, node)
        local _202_ = (comments0)[index]
        if (nil ~= _202_) then
          local existing = _202_
      ...
      local function parse_string_loop(chars, b, state)
        ...
        do
          local _212_ = {state, b}
          if ((_G.type(_212_) == "table") and ((_212_)[1] == "base") and ((_212_)[2] == 92)) then
            state0 = "backslash"
          elseif ((_G.type(_212_) == "table") and ((_212_)[1] == "base") and ((_212_)[2] == 34)) then
            state0 = "done"
          elseif ((_G.type(_212_) == "table") and ((_212_)[1] == "backslash") and ((_212_)[2] == 10)) then
      ...
      local function parse_string()
        ...
        local _216_ = (rawget(_G, "loadstring") or load)(("return " .. formatted))
        if (nil ~= _216_) then
          local load_fn = _216_
      ...
      local function parse_number(rawstr)
        ...
        if rawstr:match("^%d") then
          ...
          local _222_ = tonumber(number_with_stripped_underscores)
          if (nil ~= _222_) then
            local x = _222_
            dispatch(x)
            return true
...
package.preload["fennel.utils"] = package.preload["fennel.utils"] or function(...)
  ...
  local function stablepairs(t)
    ...
    if _105_ then
      ...
      local function _109_(_241, _242)
        return (tostring(_241) < tostring(_242))
      end
      table.sort(_107_, _109_)
      keys = _107_
    end
    local succ
    do
      ...
      for i, k in ipairs(keys) do
        local _111_, _112_ = k, keys[(i + 1)]
        if ((nil ~= _111_) and (nil ~= _112_)) then
          local k_12_auto = _111_
          local v_13_auto = _112_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
  ...
  local function copy(from, _3fto)
    ...
    for k, v in pairs((from or {})) do
      local _124_, _125_ = k, v
      if ((nil ~= _124_) and (nil ~= _125_)) then
        local k_12_auto = _124_
        local v_13_auto = _125_
        tbl_11_auto[k_12_auto] = v_13_auto
      else
      end
    end
  ...
  local function member_3f(x, tbl, _3fn)
    local _127_ = tbl[(_3fn or 1)]
    if (_127_ == x) then
      return true
    elseif (_127_ == nil) then
      return nil
    elseif true then
      local _ = _127_
  ...
  local function allpairs(tbl)
    ...
    local function allpairs_next(_, state)
      ...
      elseif next_state then
        ...
        local _129_ = getmetatable(t)
        if ((_G.type(_129_) == "table") and true) then
          local __index = (_129_).__index
          if ("table" == type(__index)) then
            t = __index
  ...
  getenv = ((os and os.getenv) or _135_)
  ...
  local function sym(str, _3fsource)
    ...
    do
      ...
      for k, v in pairs((_3fsource or {})) do
        ...
        if ((nil ~= _137_) and (nil ~= _138_)) then
          local k_12_auto = _137_
          local v_13_auto = _138_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
    ...
    return setmetatable(_136_, symbol_mt)
  end
  nil_sym = sym("nil")
  ...
  local function comment_2a(contents, _3fsource)
    local _let_141_ = (_3fsource or {})
    local filename = _let_141_["filename"]
    local line = _let_141_["line"]
    ...
    do
      ...
      for k, v in pairs((_3fsource or {})) do
        ...
        if ((nil ~= _143_) and (nil ~= _144_)) then
          local k_12_auto = _143_
          local v_13_auto = _144_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
  ...
  root = {chunk = nil, scope = nil, options = nil, reset = _152_}
  root["set-reset"] = function(_153_)
    local _arg_154_ = _153_
    local chunk = _arg_154_["chunk"]
    local scope = _arg_154_["scope"]
    local options = _arg_154_["options"]
    local reset = _arg_154_["reset"]
    root.reset = function()
  ...
  local warned = {}
  local function check_plugin_version(_155_)
    local _arg_156_ = _155_
    local name = _arg_156_["name"]
    local versions = _arg_156_["versions"]
    local plugin = _arg_156_
...
local function eval(str, options, ...)
  ...
  loader = specials["load-code"](lua_source, env, _698_(...))
  opts.filename = nil
  return loader(...)
end
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _699_ = type(v)
    if (_699_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_699_ == "table") then
...
do
  ...
  package.preload[module_name] = _702_
  _ = nil
  local env
  do
    local _703_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_703_)["utils"] = utils
    _703_["fennel"] = mod
    env = _703_
  end
