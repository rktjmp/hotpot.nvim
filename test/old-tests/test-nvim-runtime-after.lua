...
do
  local case_18_ = vim.loop.fs_access(lua_path, "R")
...
do
  local case_22_ = ((stats_a.mtime.sec == stats_b.mtime.sec) and (stats_a.mtime.nsec == stats_b.mtime.nsec))
...
if (1 ~= vim.fn.has("win32")) then
  local case_26_ = vim.loop.fs_access(lua_path, "R")
